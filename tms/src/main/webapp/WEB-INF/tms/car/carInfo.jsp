...
<body class="gray-bg">
				...
				<div class="ibox">
					<div class="ibox-content">
						...
						<div class="row">
							...
							<div class="col-md-6">
							<button id="lookMoreMaintain" class="btn btn-primary btn-block m">
								点击查看更多
							</button>
							</div>
						</div>
				...
				<div class="ibox">
					<div class="ibox-content">
						...
						<div class="row">
							...
							<div class="col-md-6">
							<button id="lookMoreIllegal" class="btn btn-primary btn-block m">
								点击查看更多
							</button>
							</div>
						</div>
	...
	var maintainPage = 1;
	$("#lookMoreMaintain").click(function(){
		maintainPage++;
		$.get("${ctx}/driverAndCarInfo/getMaintainInfo",{page:maintainPage},function(data){
			if(data.length==0){
				layer.alert("没有更多的记录", {
			...
			//$("#maintainInfoDiv").html($("#maintainInfoDiv").html()+records);
			$("#maintainInfoTable tbody:first tr:last").after(records);
		})
	})
	
	var illegalPage = 1;
	$("#lookMoreIllegal").click(function(){
		illegalPage++;
		$.get("${ctx}/driverAndCarInfo/getIllegalInfo",{page:illegalPage},function(data){
			if(data.length==0){
				layer.alert("没有更多的记录", {
					skin: 'layui-layer-molv', //样式类名
					shift:4
			...
			var records = "";
			var payIllegalFun = "";
			var c = $("#illegalInfoTable tbody:first tr:last");
			for(var i=0;i<data.length;i++){
			...
			//$("#illegalInfoDiv").html($("#illegalInfoDiv").html()+records);
			$("#illegalInfoTable tbody:first tr:last").after(records);
		})
	})
	...
	function maintainInfo(){
		$.get("${ctx}/driverAndCarInfo/getMaintainInfo",function(data){
			...
			var maintainInfoFun = "";
			var payMaintainFun = "";
			records += "<table id='maintainInfoTable' class='table table-bordered'><tbody>"
			for(var i=0;i<data.length;i++){
				maintainInfoFun = "onclick='maintainDetailInfo(" + data[i].id + ")'";
				payMaintainFun = "onclick='maintainPay("+data[i].id+","+data[i].maintainCost+")'"
				record = "<tr><td>时间："+formatDate(data[i].maintaintime)+"</td><td>原因："+data[i].maintainReson+"</td>";
	...
	function illegalInfo(){
		$.get("${ctx}/driverAndCarInfo/getIllegalInfo",function(data){
			...
			var records = "";
			var payIllegalFun = "";
			records += "<table id='illegalInfoTable' class='table table-bordered'><tbody>"
			for(var i=0;i<data.length;i++){
				payIllegalFun = "onclick='illegalPay("+data[i].id+","+data[i].illegalCost+")'"
				record = "<tr><td>时间："+formatDate(data[i].recordDate)+"</td><td>原因："+data[i].illegalReson+"</td><td>费用："+data[i].illegalCost+"元</td>";
				if(data[i].payState==0){
